...
static bool application_is_locked_update(uint8_t dataType);
static oepl_nvm_status_t oepl_app_allocate_provision_slot(size_t* img_idx);
...
static volatile oepl_hw_gpio_event_t button1_event = RISING;
static volatile oepl_hw_gpio_event_t button2_event = RISING;
...
void oepl_app_init(void)
{
  ...
  // Boot display: PIN UI if requested, otherwise kiosk user image or stock splash
  uint8_t fastboot;
  oepl_provision_init();
  oepl_button_init();
  if (oepl_provision_should_show_pin()) {
    oepl_provision_begin_pin_display();
  } else if(oepl_nvm_setting_get(OEPL_ENABLE_FASTBOOT, &fastboot, sizeof(fastboot)) == NVM_SUCCESS && fastboot != 0) {
    // Skip the boot screen
  } else {
    oepl_provision_show_user_image();
  }
  oepl_provision_handle_pending_display();
...
void oepl_app_process(void)
{
  ...
  switch(current_state) {
    ...
    case DATA_AVAILABLE:
      ...
      if(application_is_locked_update(data_to_process.AP_data.dataType)) {
        DPRINTF("Remote updates locked, ignoring datatype 0x%02X\n", data_to_process.AP_data.dataType);
      ...
      switch(data_to_process.AP_data.dataType) {
        ...
        case DATATYPE_IMG_ZLIB:
        {
          ...
          if (!oepl_provision_updates_allowed() || oepl_provision_is_active()) {
            char entered_pin[OEPL_PROVISION_PIN_LEN + 1];
            memcpy(entered_pin, data_to_process.AP_data.provisionPin, OEPL_PROVISION_PIN_LEN);
            entered_pin[OEPL_PROVISION_PIN_LEN] = '\0';
            if (!oepl_provision_authorize_session(entered_pin)) {
              PINPROV_DBG("H-A", "image_offer PIN_REJECT type=0x%02x", data_to_process.AP_data.dataType);
              DPRINTF("Image rejected: missing or wrong PIN\n");
              application_state_transition(CONNECTED);
              break;
            }
            PINPROV_DBG("H-A", "image_offer PIN_OK type=0x%02x sz=%lu",
                        data_to_process.AP_data.dataType,
                        (unsigned long)data_to_process.AP_data.dataSize);
          }
          ...
          if(nvm_status == NVM_SUCCESS) {
            ...
            if(img_meta.is_valid) {
              if(oepl_provision_is_session_authorized() || oepl_provision_is_active()) {
                PINPROV_DBG("H-C", "cache_bypass slot=%u", (unsigned)img_idx);
                DPRINTF("Provision session: force re-download (cache bypass)\n");
                ...
                img_meta.image_type = CUSTOM_IMAGE_NOCUSTOM;
                oepl_nvm_write_image_metadata(img_idx, &img_meta);
                application_state_transition(DOWNLOAD);
              } else {
                PINPROV_DBG("H-C", "cache_hit skip_download -> AWAITING_CONFIRMATION");
            ...
            uint8_t image_type = CUSTOM_IMAGE_NOCUSTOM;
            if(oepl_provision_is_session_authorized() || oepl_provision_is_active()) {
              PINPROV_DBG("H-C", "provision_slot_alloc");
              nvm_status = oepl_app_allocate_provision_slot(&img_idx);
            } else {
              nvm_status = oepl_nvm_get_free_image_slot(&img_idx, image_type);
              if(nvm_status != NVM_SUCCESS) {
            ...
            if(nvm_status == NVM_SUCCESS) {
              oepl_stored_image_hdr_t img_meta;
              bool provision_xfer = oepl_provision_is_session_authorized() || oepl_provision_is_active();
              img_meta.is_valid = false;
              img_meta.md5 = data_to_process.AP_data.dataVer;
              img_meta.size = data_to_process.AP_data.dataSize;
              img_meta.image_format = data_to_process.AP_data.dataType;
              img_meta.image_type = provision_xfer
                  ? CUSTOM_IMAGE_NOCUSTOM
                  : IMG_EXTTYPE_IMGID_FROM_EXTTYPE(data_to_process.AP_data.dataTypeArgument);
              oepl_nvm_write_image_metadata(img_idx, &img_meta);
              DPRINTF("Downloading image ID 0x%x in slot 0x%x\n", img_meta.image_type, img_idx);
              application_state_transition(DOWNLOAD);
            } else {
              ...
              if(oepl_provision_is_session_authorized() || oepl_provision_is_active()) {
                DPRINTF("Provision: skip XFC without slot, retry on next poll\n");
                application_state_transition(CONNECTED);
              } else {
                application_state_transition(AWAITING_CONFIRMATION);
              }
    ...
    case CONFIRMATION_RECEIVED:
      ...
      if(data_to_process.AP_data.dataType == DATATYPE_COMMAND_DATA) {
        switch(data_to_process.AP_data.dataTypeArgument) {
          ...
          case CMD_PROVISION_COMPLETE:
          {
            bool unlock_updates = (data_to_process.AP_data.dataVer & 0xFFU) != 0U;
            if(oepl_provision_is_provisioned()) {
              DPRINTF("Ignoring duplicate provision-complete\n");
              break;
            }
            if(oepl_provision_has_stored_image()) {
              oepl_provision_mark_provisioned(unlock_updates);
              oepl_provision_show_user_image();
            } else {
              PINPROV_DBG("H-C", "provision_complete REJECT no image");
              DPRINTF("Ignoring provision-complete without stored image\n");
              oepl_provision_show_user_image();
            }
            break;
          }
          case CMD_ENTER_PROVISION_MODE:
            oepl_provision_begin_pin_display();
            break;
          default:
        ...
        if((data_to_process.AP_data.dataTypeArgument & IMG_EXTTYPE_PRELOAD_FLAG) == 0) {
          ...
          if(nvm_status == NVM_SUCCESS && img_meta.is_valid) {
            ...
            if (oepl_provision_is_active()) {
              PINPROV_DBG("H2", "DBG1AC9CA show_image OVERWRITE_PIN slot=%u",
                          (unsigned)img_idx);
            }
            // #endregion
            PINPROV_DBG("H-C", "show_image slot=%u", (unsigned)img_idx);
            DPRINTF("Showing image\n");
            oepl_display_show_image(img_idx);
          } else if(oepl_provision_is_session_authorized() || oepl_provision_is_active()) {
            // #region agent log
            PINPROV_DBG("H2", "DBG1AC9CA show_image FALLBACK_WHILE_PIN auth=%d active=%d",
                        (int)oepl_provision_is_session_authorized(),
                        (int)oepl_provision_is_active());
            // #endregion
            PINPROV_DBG("H-C", "show_image fallback provision slot");
            oepl_provision_show_user_image();
          } else {
            PINPROV_DBG("H-C", "show_image FAIL nvm=%d valid=%d", (int)nvm_status, img_meta.is_valid);
  ...
  if(event_flags != 0) {
    if(event_flags & EVENT_FLAG_BUTTON_1) {
      ...
      if (button1_event == FALLING) {
        oepl_button_notify_press(BUTTON_1);
      } else {
        oepl_button_notify_release(BUTTON_1);
        if(current_state == CONNECTED && !oepl_provision_is_active()) {
    ...
    if(event_flags & EVENT_FLAG_BUTTON_2) {
      ...
      if (button2_event == FALLING) {
        oepl_button_notify_press(BUTTON_2);
      } else {
        oepl_button_notify_release(BUTTON_2);
        if(current_state == CONNECTED && !oepl_provision_is_active()) {
    ...
    if(event_flags & EVENT_FLAG_CONNECTED) {
      ...
      oepl_provision_on_radio_connected();
    ...
    if(event_flags & EVENT_FLAG_DISCONNECTED) {
      ...
      oepl_provision_on_radio_disconnected();
      oepl_display_draw(NULL);
...
static void application_state_transition(application_state_t new_state)
{
  ...
  if (new_state == DOWNLOAD || new_state == AWAITING_CONFIRMATION ||
      new_state == DATA_AVAILABLE || new_state == CONFIRMATION_RECEIVED) {
    PINPROV_DBG("H-A", "state %d -> %d", current_state, new_state);
  }
...
static oepl_nvm_status_t oepl_app_allocate_provision_slot(size_t* img_idx)
{
  ...
  oepl_nvm_status_t st = oepl_nvm_get_num_img_slots(&num_slots, &slot_size);
  if(st != NVM_SUCCESS) {
    DPRINTF("Provision: slot count unavailable (bulk storage missing)\n");
    return st;
  }
  if(OEPL_PROVISION_BULK_SLOT >= num_slots) {
    DPRINTF("Provision: bulk slot %u unavailable (have %u slots)\n",
            (unsigned)OEPL_PROVISION_BULK_SLOT, (unsigned)num_slots);
    return NVM_NOT_SUPPORTED;
  }

  oepl_nvm_log_image_slots();

  size_t old_idx;
  if(oepl_nvm_get_image_by_type(CUSTOM_IMAGE_NOCUSTOM, &old_idx, NULL) == NVM_SUCCESS &&
     old_idx != OEPL_PROVISION_BULK_SLOT) {
    DPRINTF("Provision: clearing stale NOCUSTOM in slot %u\n", (unsigned)old_idx);
    oepl_nvm_erase_image(old_idx);
  }

  *img_idx = OEPL_PROVISION_BULK_SLOT;
  PINPROV_DBG("H-C", "provision_force slot=%u type=0x00", (unsigned)OEPL_PROVISION_BULK_SLOT);
  DPRINTF("Provision: erase+reuse physical slot %u (NOCUSTOM)\n", (unsigned)OEPL_PROVISION_BULK_SLOT);
  return oepl_nvm_erase_image(OEPL_PROVISION_BULK_SLOT);
}

static bool application_is_locked_update(uint8_t dataType)
{
  if (oepl_provision_is_session_authorized()) {
    return !oepl_provision_accepts_content_type(dataType);
  }

  if (oepl_provision_updates_allowed()) {
    return false;
  }
...
static void application_process_data(oepl_radio_data_indication_t* data)
{
  ...
  } else if(current_state == DOWNLOAD ||
            current_state == AWAITING_CONFIRMATION ||
            current_state == CONFIRMATION_RECEIVED) {
    // C6 re-offers pending AvailDataInfo on every poll while attemptsLeft>0.
    // Ignore duplicates during an active transfer instead of rebooting.
    PINPROV_DBG("H-E", "dup_offer ignored state=%d", current_state);
    DPRINTF("Ignoring duplicate data indication during xfer (state %d)\n", current_state);
  } else {
    ...
    if(memcmp(&md5.digest[0], &img_meta.md5, sizeof(img_meta.md5)) == 0) {
      ...
      if(nvm_status != NVM_SUCCESS) {
        DPRINTF("Error making image valid\n");
                ...
                img_idx == OEPL_PROVISION_BULK_SLOT) {
        oepl_provision_on_image_downloaded(img_idx);
      } else {
        oepl_display_show_image(img_idx);
      }
      return false;
