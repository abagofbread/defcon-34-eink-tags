...
static int32_t oepl_nvm_btl_init_with_wake(void);
static bool oepl_nvm_solum_fallback_devconfig(uint8_t hwid);
static bool oepl_nvm_apply_btl_storage_layout(void);
static void oepl_nvm_reset_slot_cache(void);
static oepl_nvm_status_t oepl_nvm_ensure_bulk_storage(void);
...
static size_t oepl_nvm_num_slots_cache = 0;
static size_t oepl_nvm_slot_size_cache = 0;
...
static bool oepl_nvm_solum_fallback_devconfig(uint8_t hwid)
{
  ...
  if(tagcfg == NULL || tagcfg->hwtype != SOLUM_AUTODETECT) {
    return false;
  }

  uint32_t jedec = 0;
  uint32_t flash_size = 0;
  if(!oepl_flash_probe(&jedec, &flash_size)) {
    DPRINTF("Solum flash probe failed\n");
    return false;
  }

  const uint32_t fwu_slot = 236UL * 1024UL;
  if(flash_size <= fwu_slot + 65536UL) {
    DPRINTF("Flash too small for OTA+bulk (%luB)\n", flash_size);
    return false;
  }

  devconfig.marker = NVM3_MARKER_VALUE;
  devconfig.hwid = hwid;
  devconfig.fwu_slot_size = fwu_slot;
  devconfig.bulk_storage_base_address = fwu_slot;
  devconfig.bulk_storage_size = flash_size - fwu_slot;
  devconfig.bulk_storage_pagesize = 4096;

  DPRINTF("Solum probe layout: flash %luB bulk %luB @%08lx page %luB\n",
          flash_size,
          (unsigned long)devconfig.bulk_storage_size,
          (unsigned long)devconfig.bulk_storage_base_address,
          (unsigned long)devconfig.bulk_storage_pagesize);
  return true;
}

static void oepl_nvm_reset_slot_cache(void)
{
  oepl_nvm_num_slots_cache = 0;
  oepl_nvm_slot_size_cache = 0;
}
...
static bool oepl_nvm_apply_btl_storage_layout(void)
{
  ...
  if(flashInfo.flashInfo.partSize < 4096) {
    DPRINTF("BTL flash part size too small (%ldB)\n", flashInfo.flashInfo.partSize);
    return false;
  }
  ...
  if((status = bootloader_getStorageSlotInfo(0, &slotInfo)) != BOOTLOADER_OK) {
    DPRINTF("BTL getStorageSlotInfo failed %08lx\n", status);
    return false;
  }
  ...
  return devconfig.bulk_storage_size >= 4096;
}

static oepl_nvm_status_t oepl_nvm_ensure_bulk_storage(void)
{
  if(devconfig.marker == NVM3_MARKER_VALUE &&
     devconfig.bulk_storage_size >= 4096 &&
     devconfig.bulk_storage_pagesize > 0) {
    return NVM_SUCCESS;
  }

  if(oepl_nvm_init_default() == NVM_SUCCESS) {
    return NVM_SUCCESS;
  }

  DPRINTF("Bulk storage missing — attempting autodetect\n");
  uint8_t hwid = oepl_efr32xg22_get_oepl_hwid();
  devconfig.marker = NVM3_MARKER_VALUE;
  devconfig.hwid = hwid;

  oepl_hw_flash_wake();
  int32_t status = oepl_nvm_btl_init_with_wake();
  if(status == BOOTLOADER_OK && oepl_nvm_apply_btl_storage_layout()) {
    Ecode_t wr = nvm3_writeData(nvm3_defaultHandle, NVM3_OBJECT_ID_CONFIG, &devconfig, sizeof(devconfig));
    bootloader_deinit();
    oepl_hw_flash_deepsleep();
    if(wr == ECODE_NVM3_OK) {
      DPRINTF("Stored devconfig from BTL autodetect\n");
      oepl_nvm_reset_slot_cache();
      return NVM_SUCCESS;
    }
    DPRINTF("Failed writing devconfig, %08lX\n", wr);
    return NVM_ERROR;
  }
  ...
  if(oepl_nvm_solum_fallback_devconfig(hwid)) {
    Ecode_t wr = nvm3_writeData(nvm3_defaultHandle, NVM3_OBJECT_ID_CONFIG, &devconfig, sizeof(devconfig));
    oepl_hw_flash_deepsleep();
    if(wr == ECODE_NVM3_OK) {
      DPRINTF("Stored devconfig from SPI probe\n");
      oepl_nvm_reset_slot_cache();
      return NVM_SUCCESS;
    }
    DPRINTF("Failed writing probed devconfig, %08lX\n", wr);
  }
...
oepl_nvm_status_t oepl_nvm_factory_reset(uint8_t hwid)
{
  ...
  oepl_hw_flash_wake();
  if((status = oepl_nvm_btl_init_with_wake()) != BOOTLOADER_OK)
  {
    DPRINTF("Failed BTL init with %08lx\n", status);
    if(!oepl_nvm_solum_fallback_devconfig(hwid)) {
      goto exit;
    }
    nvm_status = nvm3_writeData(nvm3_defaultHandle, NVM3_OBJECT_ID_CONFIG, &devconfig, sizeof(devconfig));
    if(nvm_status != ECODE_NVM3_OK) {
      DPRINTF("Failed writing probe devconfig, %08lX\n", nvm_status);
      goto exit;
    }
    DPRINTF("Stored new devconfig (probe fallback)\n");
    oepl_nvm_reset_slot_cache();
...
{
  if(oepl_nvm_num_slots_cache > 0) {
    *num_slots = oepl_nvm_num_slots_cache;
    *slot_size = oepl_nvm_slot_size_cache;
    return NVM_SUCCESS;
  }
  ...
  if(devconfig.bulk_storage_pagesize == 0 ||
    ...
    if(oepl_nvm_ensure_bulk_storage() != NVM_SUCCESS) {
      DPRINTF("No bulk storage configured (page=%lu size=%lu)\n",
              (unsigned long)devconfig.bulk_storage_pagesize,
              (unsigned long)devconfig.bulk_storage_size);
      return NVM_ERROR;
    }
  ...
  oepl_nvm_slot_size_cache = raw_framesize / devconfig.bulk_storage_pagesize;
  if(raw_framesize % devconfig.bulk_storage_pagesize) {
    oepl_nvm_slot_size_cache++;
  }
  oepl_nvm_slot_size_cache *= devconfig.bulk_storage_pagesize;

  // Calculate amount of times we can fit this page-aligned size in bulk storage
  oepl_nvm_num_slots_cache = devconfig.bulk_storage_size / oepl_nvm_slot_size_cache;
  if(oepl_nvm_num_slots_cache > NVM3_OBJECT_ID_IMAGE_METADATA_MAX - NVM3_OBJECT_ID_IMAGE_METADATA_BASE) {
    oepl_nvm_num_slots_cache = NVM3_OBJECT_ID_IMAGE_METADATA_MAX - NVM3_OBJECT_ID_IMAGE_METADATA_BASE;
  }
...
void oepl_nvm_log_image_slots(void)
{
  size_t num_slots, slot_size;
  if(oepl_nvm_get_num_img_slots(&num_slots, &slot_size) != NVM_SUCCESS) {
    DPRINTF("slot inventory: unavailable\n");
    return;
  }

  DPRINTF("slot inventory: %u slots, %u bytes each\n", (unsigned)num_slots, (unsigned)slot_size);
  for(size_t i = 0; i < num_slots; i++) {
    ...
    if(nvm_status == ECODE_NVM3_ERR_KEY_NOT_FOUND) {
      ...
      DPRINTF("  [%u] read error\n", (unsigned)i);
    } else {
      DPRINTF("  [%u] type=0x%02x valid=%u size=%lu seq=%lu\n",
              (unsigned)i, imgmeta.image_type, imgmeta.is_valid,
              (unsigned long)imgmeta.size, (unsigned long)imgmeta.seqno);
    }
  }
}
...
oepl_nvm_status_t oepl_nvm_evict_oldest_image(size_t* img_idx)
{
  size_t num_slots, slot_size;
  oepl_nvm_status_t retval = oepl_nvm_get_num_img_slots(&num_slots, &slot_size);
  if(retval != NVM_SUCCESS) {
  ...
  for(size_t i = 0; i < num_slots; i++) {
    ...
    if(nvm_status == ECODE_NVM3_ERR_KEY_NOT_FOUND) {
      ...
      *img_idx = i;
      return oepl_nvm_erase_image(i);
    } else if(!found || imgmeta.seqno < oldest_seq) {
      oldest_idx = i;
      oldest_seq = imgmeta.seqno;
      found = true;
    }
  }
  ...
  *img_idx = oldest_idx;
  DPRINTF("Evicting oldest image (seq 0x%x) from slot 0x%x\n", (unsigned)oldest_seq, (unsigned)oldest_idx);
  return oepl_nvm_erase_image(oldest_idx);
}
...
oepl_nvm_status_t oepl_nvm_erase_image(size_t img_idx)
{
  ...
  int32_t btl_status;
  oepl_hw_flash_wake();
  if((btl_status = oepl_nvm_btl_init_with_wake()) != BOOTLOADER_OK) {
    DPRINTF("Failed BTL init with %08lx\n", btl_status);
    uint32_t addr = devconfig.bulk_storage_base_address + img_idx * slot_size;
    if(!oepl_flash_erase_range(addr, slot_size)) {
      DPRINTF("HAL erase fallback failed for slot 0x%x\n", (unsigned)img_idx);
      retval = NVM_ERROR;
      goto exit;
    }
    DPRINTF("HAL erase fallback OK for slot 0x%x\n", (unsigned)img_idx);
  } else {
  ...
  exit:
  oepl_hw_flash_deepsleep();
  if(btl_status != BOOTLOADER_OK && retval != NVM_SUCCESS) {
    return NVM_ERROR;
  } else {
    return retval;
  }
...
oepl_nvm_status_t oepl_nvm_write_image_bytes(size_t img_idx, size_t offset, const uint8_t* bytes, size_t length)
{
  ...
  int32_t btl_status;
  uint32_t addr = devconfig.bulk_storage_base_address + img_idx * slot_size + offset;
  bool written = false;

  if((btl_status = oepl_nvm_btl_init_with_wake()) == BOOTLOADER_OK) {
    DPRINTF("Write %lu to addr 0x%08lx\n", (unsigned long)length, (unsigned long)addr);
    btl_status = bootloader_writeRawStorage(addr, (uint8_t*)bytes, length);
    bootloader_deinit();
    written = (btl_status == BOOTLOADER_OK);
    if(!written) {
      DPRINTF("BTL write failed with %08lx\n", btl_status);
    }
  } else {
    DPRINTF("Failed BTL init with %08lx\n", btl_status);
  }
  ...
  if(!written) {
    if(!oepl_flash_write_range(addr, bytes, length)) {
      DPRINTF("HAL write fallback failed for slot 0x%x offset %lu\n",
              (unsigned)img_idx, (unsigned long)offset);
      oepl_hw_flash_deepsleep();
      return NVM_ERROR;
    }
    DPRINTF("HAL write fallback OK %lu bytes @ %08lx\n", (unsigned long)length, (unsigned long)addr);
  }
