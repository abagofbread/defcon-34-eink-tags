...
#include "common/defcon34_logo.h"
...
static void add_rendered_content_provisioning(void);
...
static void add_rendered_content_kiosk_default(void);
static void add_checkerboard_background(uint16_t tile_px);
...
void oepl_display_init(oepl_efr32xg22_displayparams_t* driverconfig)
{
  ...
  oepl_display_diag_cache_params(driverconfig);
  DPRINTF("init driver %ux%u work %ux%u colors %u\n",
          (unsigned)displayparams.x_res, (unsigned)displayparams.y_res,
          (unsigned)displayparams.x_res_effective, (unsigned)displayparams.y_res_effective,
          (unsigned)displayparams.num_colors);
...
void oepl_display_show_infoscreen(oepl_display_infoscreen_t screen)
{
  ...
  switch(screen) {
    ...
    case INFOSCREEN_BOOT:
      ...
      break;
    case INFOSCREEN_PROVISIONING:
    case INFOSCREEN_KIOSK_DEFAULT:
      imgtype = 0xFF;
      break;
  ...
  oepl_stored_image_hdr_t override_meta;
  DPRINTF("Checking for image ID 0x%x\n", imgtype);
  if(oepl_nvm_get_image_by_type(imgtype, &override_imgidx, &img_size) == NVM_SUCCESS &&
     oepl_nvm_read_image_metadata(override_imgidx, &override_meta) == NVM_SUCCESS &&
     override_meta.is_valid) {
    DPRINTF("Found at idx 0x%x\n", override_imgidx);
    oepl_display_diag_log_show("show stored override", screen, (int)override_imgidx);
    oepl_display_show_image(override_imgidx);
    return;
  } else if(imgtype != 0xFF) {
    DPRINTF("No valid stored override for image ID 0x%x\n", imgtype);
  }

  oepl_display_diag_log_show("render", screen, -1);
  ...
  switch(screen) {
    ...
    case INFOSCREEN_BOOT:
      ...
      break;
    case INFOSCREEN_PROVISIONING:
      add_rendered_content_provisioning();
      break;
    case INFOSCREEN_BOOT_FOUND_AP:
    ...
    case INFOSCREEN_WAKEUP_RFWAKE:
      ...
      return;
    case INFOSCREEN_KIOSK_DEFAULT:
      add_rendered_content_kiosk_default();
      break;
  }
...
void oepl_display_draw(oepl_display_draw_done_cb_t cb)
{
  ...
  } else if(current_state.image.image_idx >= 0) {
    oepl_display_show_image((size_t)current_state.image.image_idx);
  }
}
...
#ifndef OEPL_KIOSK_AP_BASE
/* SoftAP portal when phone joins the OEPL hotspot (ESP32 default). No trailing slash. */
#define OEPL_KIOSK_AP_BASE "http://192.168.4.1"
#endif
...
static void add_rendered_content_provisioning(void)
{
  ...
  const char *pin = oepl_provision_get_pin();
  /* Version 4 (~65B URL). Scale for scanability on 2.6". */
  const uint8_t qr_ver = 4;
  const uint8_t qr_scale = (xres >= 360 && yres >= 184) ? 3 : 2;
  const uint16_t qr_px = (uint16_t)((17 + 4 * qr_ver) * qr_scale);
  const uint16_t qr_x = (xres > qr_px + 4) ? (uint16_t)(xres - qr_px - 4) : 0;
  const uint16_t qr_y = 4;

  C_epdSetFont(&FreeSansBold18pt7b);
  C_epdPrintf(5, 5, COLOR_BLACK, ROTATE_0, "Setup PIN");
  C_epdSetFont(&FreeSansBold24pt7b);
  C_epdPrintf(5, 45, COLOR_RED, ROTATE_0, "%s", pin);
  C_epdSetFont(&FreeSans9pt7b);
  C_epdPrintf(5, yres - 40, COLOR_BLACK, ROTATE_0, "Scan QR to program");
  C_epdPrintf(5, yres - 20, COLOR_BLACK, ROTATE_0, "%02X%02X%02X%02X%02X%02X%02X%02X",
              mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], mac[6], mac[7]);
  C_addQR(qr_x, qr_y, qr_ver, qr_scale,
          OEPL_KIOSK_AP_BASE "/program.html?mac=%02X%02X%02X%02X%02X%02X%02X%02X&pin=%s",
          mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], mac[6], mac[7], pin);
}
...
static void add_rendered_content_splash(void)
{
  ...
  switch (hwid) {
    ...
    case SOLUM_M3_BWR_22_LITE:
      ...
      break;
    case SOLUM_M3_BW_26: {
      const uint16_t logo_w = (uint16_t)dc34_logo[0] | ((uint16_t)dc34_logo[1] << 8);
      const uint16_t logo_x = (xres > logo_w) ? (uint16_t)((xres - logo_w) / 2) : 0;
      C_addFlashImage(logo_x, 6, COLOR_BLACK, ROTATE_0, dc34_logo);
      C_epdSetFont(&FreeSans9pt7b);
      C_epdPrintf(5, yres - 52, COLOR_BLACK, ROTATE_0, "Triple-tap button for PIN");
      C_epdPrintf(5, yres - 36, COLOR_BLACK, ROTATE_0, "Enter PIN to program");
      C_epdPrintf(5, yres - 20, COLOR_BLACK, ROTATE_0, "FW: %04X-%s", fw, suffix);
      break;
    }
...
static void add_checkerboard_background(uint16_t tile_px)
{
  ...
  for (uint16_t y = 0; y < yres; y += tile_px) {
    uint16_t th = tile_px;
    if (y + th > yres) {
      th = (uint16_t)(yres - y);
    }
    for (uint16_t x = 0; x < xres; x += tile_px) {
      uint16_t tw = tile_px;
      if (x + tw > xres) {
        tw = (uint16_t)(xres - x);
      }
      bool alt = (((x / tile_px) + (y / tile_px)) & 1) == 0;
      if (alt) {
        C_drawMask(x, y, tw, th, COLOR_BLACK);
      } else if (num_colors >= 3) {
        C_drawMask(x, y, tw, th, COLOR_RED);
      }
    }
  }
}
...
static void add_rendered_content_kiosk_default(void)
{
  ...
  /* Text-only boot screen — no drawMask tiles (calloc failures = blank display). */
  C_epdSetFont(&FreeSansBold18pt7b);
  C_epdPrintf(5, 5, COLOR_BLACK, ROTATE_0, "Kiosk");
  C_epdSetFont(&FreeSans9pt7b);
  C_epdPrintf(5, 35, COLOR_BLACK, ROTATE_0, "OpenEPaperLink");
  if (num_colors >= 3) {
    C_epdPrintf(5, 55, COLOR_RED, ROTATE_0, "2.6\" BWRY");
  }
  C_epdPrintf(5, yres - 40, COLOR_BLACK, ROTATE_0, "FW: %04X-%s", fw, suffix);
  C_epdPrintf(5, yres - 20, COLOR_BLACK, ROTATE_0, "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X",
              mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], mac[6], mac[7]);
...
void oepl_display_show_kiosk_default(void)
{
  oepl_display_show_infoscreen(INFOSCREEN_KIOSK_DEFAULT);
}
