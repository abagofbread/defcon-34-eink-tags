...
uint8_t oepl_efr32xg22_get_oepl_hwid(void)
{
  ...
  if(tagcfg->hwtype == SOLUM_AUTODETECT) {
    ...
    if (*((uint8_t*) USERDATA_BASE) != 0xfa || *((uint8_t*) (USERDATA_BASE + 1)) != 0x62) {
...
bool oepl_efr32xg22_get_displayparams(oepl_efr32xg22_displayparams_t* displayparams)
{
  ...
  if(tagcfg->display->type == EPD_SOLUM_AUTODETECT) {
    ...
    /* After chip erase userdata is often missing; fall back to common 2.6" BWRY. */
    if (*((uint8_t*) USERDATA_BASE) != 0xfa || *((uint8_t*) (USERDATA_BASE + 1)) != 0x62) {
      solum_ctrltype = 0x20;
      solum_colortype = 0x03;
      solum_xres = 360;
      solum_yres = 184;
      solum_tagtype = STYPE_SIZE_26_BWRY;
    }
