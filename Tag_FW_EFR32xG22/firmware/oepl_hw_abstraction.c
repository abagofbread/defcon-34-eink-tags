...
static void oepl_hw_solum_extflash_wake(const oepl_efr32xg22_tagconfig_t* tagconfig)
{
  if(tagconfig == NULL || tagconfig->nfc == NULL) {
    return;
  }
...
static void oepl_hw_solum_extflash_sleep(const oepl_efr32xg22_tagconfig_t* tagconfig)
{
  if(tagconfig == NULL || tagconfig->nfc == NULL) {
    return;
  }
...
void oepl_hw_init(void)
{
  ...
  // Setup NFC / external-flash power (Solum routes SPI flash via I2C mux)
  if(tagconfig->nfc) {
    if(tagconfig->hwtype == SOLUM_AUTODETECT) {
      oepl_hw_solum_extflash_wake(tagconfig);
    }
  }
  ...
  oepl_nvm_status_t slot_status = oepl_nvm_get_num_img_slots(&slots, &slot_size);
  if(slot_status == NVM_SUCCESS) {
    DPRINTF("Have %u image slots of %u bytes\n", (unsigned)slots, (unsigned)slot_size);
  } else {
    DPRINTF("Image slots unavailable (bulk storage not configured)\n");
  }

  if(tagconfig->nfc && tagconfig->hwtype == SOLUM_AUTODETECT &&
     slot_status == NVM_SUCCESS) {
    oepl_hw_solum_extflash_sleep(tagconfig);
  }
  ...
  if(!is_valid) {
    ...
    oepl_display_diag_log_boot(hwid, NULL);
    while(1) {
      sl_power_manager_sleep();
    }
  }

  oepl_display_diag_log_boot(hwid, &displayconfig);
...
static void gpioint_cb(uint8_t pin, void* ctx)
{
  ...
  if(button1_hwval && (pin == (button1_hwval & 0xF))) {
    GPIO_Port_TypeDef port = (GPIO_Port_TypeDef)((button1_hwval & 0x70) >> 4);
    oepl_hw_gpio_event_t event = (GPIO_PinInGet(port, pin) == 0) ? FALLING : RISING;
    ((oepl_hw_gpio_cb_t)ctx)(BUTTON_1, event);
  }
  if(button2_hwval && (pin == (button2_hwval & 0xF))) {
    GPIO_Port_TypeDef port = (GPIO_Port_TypeDef)((button2_hwval & 0x70) >> 4);
    oepl_hw_gpio_event_t event = (GPIO_PinInGet(port, pin) == 0) ? FALLING : RISING;
    ((oepl_hw_gpio_cb_t)ctx)(BUTTON_2, event);
  }
...
bool oepl_hw_gpio_is_pressed(oepl_hw_gpio_channel_t channel)
{
  uint8_t hwval = 0;
  switch (channel) {
    case BUTTON_1:
  ...
  GPIO_Port_TypeDef port = (GPIO_Port_TypeDef)((hwval & 0x70) >> 4);
  unsigned int pin = hwval & 0xF;
...
{
  static char suffix_buf[24];
#if defined(OEPL_FW_SUFFIX)
  snprintf(suffix_buf, sizeof(suffix_buf), "%s%u", OEPL_FW_SUFFIX, (unsigned)oepl_hw_get_swversion());
  return suffix_buf;
...
void oepl_hw_flash_deepsleep(void)
{
  ...
  const oepl_efr32xg22_tagconfig_t* tagconfig = oepl_efr32xg22_get_config();
  if(tagconfig != NULL && tagconfig->nfc != NULL && tagconfig->hwtype == SOLUM_AUTODETECT) {
    oepl_hw_solum_extflash_sleep(tagconfig);
  }
}
