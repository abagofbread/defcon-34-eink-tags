      ...
      if(try_ingest_packet(expected_packettypes, sizeof(expected_packettypes), &f, &payload_type, &payload, &payload_size, &packet_info, &packet_details)) {
        ...
        if(payload_size == sizeof(struct AvailDataInfo)) {
          if(checksum_check(payload, sizeof(struct AvailDataInfo))) {
            oepl_radio_action_t cb_result = NO_ACTION;
            ...
            if(current_state_data.polling.result.AP_data.dataType != DATATYPE_NOUPDATE) {
              const struct AvailDataInfo *adi = &current_state_data.polling.result.AP_data;
              PINPROV_DBG("H-E", "ADInfo type=0x%02x sz=%lu pin=%.6s",
                          adi->dataType,
                          (unsigned long)adi->dataSize,
                          adi->provisionPin);
            ...
            num_polls++;
            if(current_state_data.polling.result.AP_data.dataType == DATATYPE_NOUPDATE) {
              if(oepl_provision_wants_fast_poll()) {
                uint32_t seconds = OEPL_PROVISION_FAST_POLL_S;
                if(current_state_data.polling.result.AP_data.nextCheckIn >= 0x8000) {
                  uint32_t ap_seconds = current_state_data.polling.result.AP_data.nextCheckIn - 0x8000;
                  if(ap_seconds > 0 && ap_seconds < seconds) {
                    seconds = ap_seconds;
                  }
                }
                DPRINTF("NOUPDATE during provision — repoll in %lus\n", (unsigned long)seconds);
                schedule_next_poll(seconds);
              } else if(current_state_data.polling.result.AP_data.nextCheckIn >= 0x8000) {
                uint32_t seconds = current_state_data.polling.result.AP_data.nextCheckIn - 0x8000;
                if(seconds < POLL_INTERVAL_BASE_TIME) {
                  seconds = POLL_INTERVAL_BASE_TIME;
                }
                schedule_next_poll(seconds);
              } else {
                uint32_t minutes = current_state_data.polling.result.AP_data.nextCheckIn;
                if(minutes == 0) {
                  minutes = 1;
                }
                schedule_next_poll(minutes * 60);
              }
            } else if(current_state_data.polling.result.AP_data.dataType == DATATYPE_COMMAND_DATA) {
              schedule_next_poll(oepl_provision_wants_fast_poll() ? OEPL_PROVISION_FAST_POLL_S : POLL_INTERVAL_BASE_TIME);
            } else {
              schedule_next_poll(0);
            }
            ...
            PINPROV_DBG("H-E", "ADInfo BAD checksum");
            DPRINTF("Wrong checksum\n");
          }
        } else {
          PINPROV_DBG("H-E", "ADInfo BAD size=%u want=%u",
                      (unsigned)payload_size, (unsigned)sizeof(struct AvailDataInfo));
...
oepl_radio_error_t oepl_radio_request_datablock(oepl_datablock_descriptor_t db)
{
  ...
  PINPROV_DBG("H-A", "TX RQB block=%lu file_sz=%lu", (unsigned long)db.idx, (unsigned long)db.file.filesize);
...
static void send_poll(uint8_t channel, bool is_short)
{
  ...
  if(rx_state != AWAIT_DATAINFO) {
    ...
    if(!is_short) {
      ...
      oepl_provision_fill_poll(availreq);
...
static void schedule_next_poll(size_t timeout_s)
{
  ...
  if(timeout_s == 0) {
    if(oepl_provision_wants_fast_poll()) {
      checkin_time = OEPL_PROVISION_FAST_POLL_S;
    } else if(oepl_nvm_setting_get(OEPL_MIN_CHECKIN_INTERVAL_S, &checkin_time, sizeof(checkin_time)) != NVM_SUCCESS) {
      checkin_time = POLL_INTERVAL_BASE_TIME;
    }

    if(!oepl_provision_wants_fast_poll() && checkin_time < POLL_INTERVAL_BASE_TIME) {
      checkin_time = POLL_INTERVAL_BASE_TIME;
    }
