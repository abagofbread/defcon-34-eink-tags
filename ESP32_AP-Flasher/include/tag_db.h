...
class tagRecord {
   public:
    tagRecord() : mac{0}, version(0), alias(""), lastseen(0), nextupdate(0), contentMode(0), pendingCount(0), md5{0}, expectedNextCheckin(0), modeConfigJson(""), LQI(0), RSSI(0), temperature(0), batteryMv(0), hwType(0), wakeupReason(0), capabilities(0), lastfullupdate(0), isExternal(false), apIp(IPAddress(0, 0, 0, 0)), pendingIdle(0), rotate(0), lut(0), tagSoftwareVersion(0), currentChannel(0), dataType(0), filename(""), data(nullptr), len(0), invert(0), updateCount(0), updateLast(0), provisioned(0), updatesUnlocked(0), pinSessionOpen(0), pendingUnlockOnComplete(0), pinSessionStarted(0), lastCustomMode(0) {}
    ...
    uint8_t provisioned;
    uint8_t updatesUnlocked;
    uint8_t pinSessionOpen;
    uint8_t pendingUnlockOnComplete;
    uint32_t pinSessionStarted;
    uint8_t lastCustomMode;
...
extern bool tagUsesKioskWorkflow(const tagRecord* taginfo);
extern bool tagContentPushAllowed(const tagRecord* taginfo);
extern bool tagNeedsPinForImagePush(const tagRecord* taginfo);
extern void expireStaleProvisionSessions();
extern bool tagAwaitingProvision(const tagRecord* taginfo);
extern void updateTagProvisionState(tagRecord* taginfo, uint8_t customMode, const uint8_t* reserved);
