...
struct PendingItem {
    ...
    uint8_t* data;
    uint32_t len;
    uint8_t highestBlockDelivered = 0xFF;
};
...
extern bool kioskImageIsStaged(const uint8_t* mac);
extern bool pushKioskStagedImage(const uint8_t* mac);
extern bool pushKioskImageBuffer(const uint8_t* mac, uint8_t* data, size_t len, const char* pin = nullptr);
extern bool stageKioskFromJpeg(const uint8_t* mac, const String& jpgPath, uint8_t dither = 1);
