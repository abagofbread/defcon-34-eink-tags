...
function processTags(tagArray) {
	for (const element of tagArray) {
		...
		if (element.RSSI && element.hwType) {
		...
		if (element.awaitingPin || element.needsUploadPin || !element.provisioned || element.updatesLocked) {
			div.classList.add("tag-unprovisioned");
			if (element.programming) {
				$('#tag' + tagmac + ' .nextcheckin').innerHTML = "<font color=cyan>Programming...</font>";
			} else if (element.provisionState === 'pin_visible' || element.awaitingPin) {
				$('#tag' + tagmac + ' .nextcheckin').innerHTML = "<font color=orange>PIN on tag</font>";
			} else {
				$('#tag' + tagmac + ' .nextcheckin').innerHTML = "<font color=orange>Enter PIN to upload</font>";
			}
		} else {
			div.classList.remove("tag-unprovisioned");
		}
		if (element.updatesLocked) {
			div.classList.add("tag-locked");
			if (!element.awaitingPin && element.provisionState === 'locked') {
				$('#tag' + tagmac + ' .nextcheckin').innerHTML = "<font color=#888>Updates locked</font>";
			}
		} else {
			div.classList.remove("tag-locked");
		}
	...
	GroupSortFilter();
	if ($('#configbox')?.open && $('#cfgmac')?.dataset?.mac) {
		const openMac = $('#cfgmac').dataset.mac;
		if (tagDB[openMac]) {
			updateProvisionDialog(tagDB[openMac]);
		}
	}
}
...
function updateProvisionDialog(tagdata) {
	const needsPin = tagdata.awaitingPin === true;
	const mac = $('#cfgmac').dataset.mac;
	const browserImage = window.kioskImageBuffers?.[mac];
	const hasBrowserImage = browserImage instanceof Uint8Array && browserImage.length > 0;
	const isKioskTag = tagdata.needsUploadPin || !tagdata.provisioned || tagdata.updatesLocked || tagdata.awaitingPin ||
		tagdata.provisionState === 'kiosk_idle' || tagdata.provisionState === 'pin_visible' || tagdata.provisionState === 'locked';
	const showProvision = isKioskTag || tagdata.provisionState !== 'idle' || hasBrowserImage;

	$('#provisionbox').style.display = showProvision ? 'block' : 'none';
	$('#cfgpin').value = '';
	$('#cfgunlock').checked = false;

	const statusLabels = {
		pin_visible: 'PIN is on the tag — enter it below to upload',
		kiosk_idle: 'Triple-tap or Show PIN to read the tag PIN, then enter it to upload',
		programming: 'Image transfer in progress',
		locked: 'Updates locked — enter PIN to replace image',
		idle: hasBrowserImage ? 'Image ready in browser' : ''
	};
	$('#provisionstatus').textContent = statusLabels[tagdata.provisionState] || statusLabels.idle;

	if ($('#kioskstagestatus')) {
		$('#kioskstagestatus').innerHTML = hasBrowserImage
			? `<strong>Image ready in browser</strong> (${browserImage.length} bytes). <a href="program.html?mac=${mac}">Open program page</a>`
			: 'Prepare an image on the <a href="program.html?mac=' + mac + '">program page</a> (encoded in your browser, not on the AP).';
	}

	$('#cfgstartprovision').style.display = 'inline-block';
	const showPinEntry = isKioskTag || hasBrowserImage || tagdata.provisionState === 'programming';
	$('#cfgpin').parentNode.parentNode.style.display = showPinEntry ? '' : 'none';
	$('#cfgprovision').parentNode.style.display = showPinEntry ? '' : 'none';
	$('#cfgunlock').parentNode.parentNode.style.display = showPinEntry ? '' : 'none';
	$('#savebar').style.display = needsPin ? 'none' : 'flex';

	$('#cfgcontent').parentNode.style.display = 'flex';
}

async function loadContentCard(mac) {
	$('#cfgmac').innerHTML = mac;
	$('#cfgmac').dataset.mac = mac;
	$('#customoptions').innerHTML = "<p>Loading…</p>";
	$('#configbox').showModal();
	try {
		const response = await fetch("get_db?mac=" + mac);
		const data = await response.json();
		if (!data.tags || !data.tags[0]) {
			$('#customoptions').innerHTML = "<p>Tag not found.</p>";
			return;
		}
		...
		await getTagtype(tagdata.hwType);
		updateProvisionDialog(tagdata);

		if (populateSelectTag(tagdata.hwType, tagdata.capabilities)) {
			$('#cfgcontent').value = tagdata.contentMode || 22;
			if (!$('#cfgcontent').value || $('#cfgcontent').value === '0') {
				$('#cfgcontent').value = 22;
			}
			...
			$('#customoptions').innerHTML = "<p>Could not load content options for this tag type.</p>";
		}
		...
		$('#customoptions').innerHTML = "<p>Failed to load tag: " + error + "</p>";
	}
}
...
$('#cfgstartprovision').onclick = function () {
	const mac = $('#cfgmac').dataset.mac;
	let formData = new FormData();
	formData.append("mac", mac);
	fetch("start_provision", {
		method: "POST",
		body: formData
	})
		.then(response => response.text().then(text => ({ ok: response.ok, text })))
		.then(({ ok, text }) => showMessage(text, !ok))
		.catch(error => showMessage('Error: ' + error, true));
};

$('#cfgprovision').onclick = function () {
	const mac = $('#cfgmac').dataset.mac;
	const pin = $('#cfgpin').value.trim();
	if (!/^[0-9]{6}$/.test(pin)) {
		const msg = 'Enter the 6-digit PIN shown on the tag';
		showMessage(msg, true);
		if ($('#provisionstatus')) $('#provisionstatus').textContent = msg;
		return;
	}
	const browserImage = window.kioskImageBuffers?.[mac];
	if (!(browserImage instanceof Uint8Array) || browserImage.length === 0) {
		const msg = 'Prepare an image on the program page first';
		showMessage(msg, true);
		if ($('#provisionstatus')) $('#provisionstatus').textContent = msg;
		return;
	}
	$('#cfgprovision').disabled = true;
	let formData = new FormData();
	formData.append("mac", mac);
	formData.append("pin", pin);
	formData.append("unlock", $('#cfgunlock').checked ? "1" : "0");
	formData.append("image", new Blob([browserImage]), "kiosk.raw");
	fetch("provision_tag", {
		method: "POST",
		body: formData
	})
		...
		.then(({ ok, text }) => {
			showMessage(text, !ok);
			if ($('#provisionstatus')) {
				$('#provisionstatus').textContent = ok ? text : ('Error: ' + text);
				$('#provisionstatus').style.color = ok ? '' : 'crimson';
			}
			if (ok) {
				if (window.provisionPrompted) delete window.provisionPrompted[mac];
			}
			loadContentCard(mac);
		})
		.catch(error => {
			showMessage('Error: ' + error, true);
			if ($('#provisionstatus')) $('#provisionstatus').textContent = 'Error: ' + error;
		})
		.finally(() => {
			if ($('#cfgprovision')) $('#cfgprovision').disabled = false;
		});
};
...
$('#paintbutton').onclick = function () {
	if (paintShow) {
		...
		const mac = $('#cfgmac').dataset.mac;
		const hwtype = $('#cfgmac').dataset.hwtype || $('#tag' + mac)?.dataset?.hwtype;
		const tagType = tagTypes[hwtype];
		if (!tagType) {
			showMessage('Tag type not loaded yet — try again in a moment', true);
			return;
		}
		const width = tagType.width ?? 0;
		const height = tagType.height ?? 0;
		const start = () => startPainter(mac, width, height, tagType);
		if (paintLoaded) {
			start();
		} else {
...
let fileListCache = { data: null, fetchedAt: 0 };
const FILE_LIST_CACHE_MS = 60000;

function fetchFileList(dirs, extensions) {
	const now = Date.now();
	const cacheKey = dirs.join(',') + '|' + extensions.join(',');
	if (fileListCache.key === cacheKey && fileListCache.data && (now - fileListCache.fetchedAt) < FILE_LIST_CACHE_MS) {
		return Promise.resolve(fileListCache.data);
	}
	return Promise.all(dirs.map(dir =>
		fetch('edit?list=' + encodeURIComponent(dir))
			.then(response => response.json())
			.then(items => items.map(item => {
				if (item.type !== 'file') return item;
				const base = dir.endsWith('/') ? dir.slice(0, -1) : dir;
				return { ...item, name: base + '/' + item.name };
			}))
			.catch(() => [])
	)).then(results => {
		...
		fileListCache = { key: cacheKey, data: filtered, fetchedAt: now };
		return filtered;
	});
}

function populateFileSelect(input, element, obj, extensions, dirs) {
	const optionElement = document.createElement("option");
	optionElement.value = "";
	optionElement.text = "";
	input.appendChild(optionElement);
	fetchFileList(dirs, extensions).then(files => {
		files.forEach(item => {
			const optionElement = document.createElement("option");
			optionElement.value = item.name;
			optionElement.text = item.name;
			if (obj[element.key] === item.name) {
		...
		extraoptions?.forEach(element => {
			...
			switch (element.type) {
				...
				case 'jsonfile':
					...
					if (element.type == 'jpgfile') {
						populateFileSelect(input, element, obj, ['.jpg', '.jpeg'], ['/temp']);
					} else if (element.type == 'binfile') {
						populateFileSelect(input, element, obj, ['.bin'], ['/temp', '/']);
					} else {
						populateFileSelect(input, element, obj, ['.json'], ['/']);
					}
					break;
				case 'select':
				case 'chanselect':
...
function populateSelectTag(hwtype, capabilities) {
	...
	cardconfig.forEach(item => {
		...
		if (tagType.contentids?.includes(item.id) && (capabilities & capcheck || capcheck == 0) && (apConfig.savespace == 0 || !item.properties?.includes("savespace"))) {
	...
	for (let i = 0; i < 4; i++) {
		if (i == 0 || tagType.width == tagType.height || (i == 2)) {
			option = document.createElement("option");
			option.value = i;
...
function drawCanvas(buffer, canvas, hwtype, tagmac, doRotate) {
	...
	if (tagTypes[hwtype].bpp == 16) {
		...
		const planeBytes = offsetRed || data.length;
		let pixelValue = 0;
		const colorTable = tagTypes[hwtype].colortable;
...
const downloadTagtype = async (hwtype) => {
	try {
		if (apConfig.offline) {
			console.log("offline AP: tagtype " + hwtype + " not bundled on device");
...
async function getTagtype(hwtype) {
	// hwType 0 is "not reported yet" — do NOT load tagtypes/00.json (M2 1.54").
	if (!hwtype) {
		return { name: 'hardware type pending', width: 0, height: 0, bpp: 0, rotatebuffer: 0, colortable: [], busy: false };
	}
