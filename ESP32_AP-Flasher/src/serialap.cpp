...
static bool radioLinkUsable() {
    if (apInfo.state == AP_STATE_NORADIO || apInfo.state == AP_STATE_FLASHING) {
        return false;
    }
    if (apInfo.isOnline) {
        return true;
    }
    return lastAPActivity != 0 && (millis() - lastAPActivity) < AP_ACTIVITY_MAX_INTERVAL;
}

static void noteRadioActivity() {
    lastAPActivity = millis();
    if (apInfo.state != AP_STATE_FLASHING && !apInfo.isOnline) {
...
bool sendDataAvail(struct pendingData* pending) {
    if (apInfo.state == AP_STATE_NORADIO) return true;
    if (!radioLinkUsable()) return false;
