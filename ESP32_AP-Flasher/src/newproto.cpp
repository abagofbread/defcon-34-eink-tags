...
static bool tagUpdatesAllowed(const tagRecord* taginfo) {
    return tagContentPushAllowed(taginfo);
}

static bool isAllowedWhenLockedQueueItem(const PendingItem* queueItem) {
    return queueItem != nullptr &&
           queueItem->pendingdata.availdatainfo.dataType == DATATYPE_COMMAND_DATA;
}

static bool pendingHasProvisionPin(const struct pendingData* pending) {
    if (pending == nullptr) {
        return false;
    }
    return pending->availdatainfo.provisionPin[0] >= '0' &&
           pending->availdatainfo.provisionPin[0] <= '9';
}

static bool isProvisionImageTransfer(uint8_t dataType) {
    switch (dataType) {
...
static bool pendingXferComplete(const PendingItem* queueItem) {
    ...
    return queueItem->highestBlockDelivered != 0xFF &&
           queueItem->highestBlockDelivered >= totalblocks - 1;
}
...
void prepareDataAvail(const uint8_t* dst) {
    ...
    if (!tagUpdatesAllowed(taginfo)) return;
...
bool prepareDataAvail(String& filename, uint8_t dataType, uint8_t dataTypeArgument, const uint8_t* dst, uint16_t nextCheckin, bool resend) {
    ...
    if (!tagUpdatesAllowed(taginfo)) return true;
...
void processBlockRequest(struct espBlockRequest* br) {
    ...
    if (queueItem->highestBlockDelivered == 0xFF || br->blockId > queueItem->highestBlockDelivered) {
        queueItem->highestBlockDelivered = br->blockId;
    }
    char buffer[150];
...
void processXferComplete(struct espXferComplete* xfc, bool local) {
    ...
    memset(md5bytes, 0, sizeof(md5bytes));

    tagRecord* taginfo = tagRecord::findByMAC(xfc->src);
    bool provisionXfer = false;
    uint8_t unlockOnComplete = 0;
    if (taginfo != nullptr && taginfo->pinSessionOpen != 0) {
        unlockOnComplete = taginfo->pendingUnlockOnComplete;
    }

    PendingItem* queueItem = getQueueItem(xfc->src);
    bool xferComplete = pendingXferComplete(queueItem);
    if (queueItem != nullptr) {
        provisionXfer = (taginfo != nullptr && taginfo->pinSessionOpen != 0) &&
                        isProvisionImageTransfer(queueItem->pendingdata.availdatainfo.dataType);
        if (!provisionXfer || xferComplete) {
    ...
    if (taginfo != nullptr) {
        ...
        if (taginfo->pinSessionOpen != 0) {
            if (provisionXfer) {
                if (xferComplete) {
                    const bool wasProvisioned = taginfo->provisioned != 0;
                    taginfo->provisioned = 1;
                    taginfo->updatesUnlocked = unlockOnComplete;
                    if (!wasProvisioned && taginfo->isExternal == false) {
                        uint8_t payload[12] = {0};
                        payload[0] = taginfo->updatesUnlocked ? 1 : 0;
                        sendTagCommand(xfc->src, CMD_PROVISION_COMPLETE, true, payload);
                    }
...
void processDataReq(struct espAvailDataReq* eadr, bool local, IPAddress remoteIP) {
    ...
    if (taginfo == nullptr) {
        ...
        updateTagProvisionState(taginfo, eadr->adr.customMode, eadr->adr.reserved);
...
void checkQueue(const uint8_t* targetMac) {
    tagRecord* taginfo = tagRecord::findByMAC(targetMac);
    uint16_t queueCount = countQueueItem(targetMac);
    if (queueCount > 0) {
        ...
        if (taginfo != nullptr &&
            queueItem->pendingdata.availdatainfo.dataType == DATATYPE_COMMAND_DATA &&
            queueItem->pendingdata.availdatainfo.dataTypeArgument == CMD_PROVISION_COMPLETE &&
            taginfo->provisioned != 0) {
            dequeueItem(targetMac);
            taginfo->pendingCount = countQueueItem(targetMac);
            if (countQueueItem(targetMac) > 0) {
                checkQueue(targetMac);
            }
            return;
        }
        if (!tagUpdatesAllowed(taginfo) &&
            !isAllowedWhenLockedQueueItem(queueItem) &&
            !pendingHasProvisionPin(&queueItem->pendingdata)) {
            if (taginfo != nullptr && taginfo->isExternal == false) {
                prepareIdleReq(targetMac, 1);
            }
            return;
        }
        Serial.printf("queue: total %d elements\r\n", pendingQueue.size());
        if (queueCount > 1 || (taginfo != nullptr && tagUsesKioskWorkflow(taginfo))) {
            queueItem->pendingdata.availdatainfo.nextCheckIn = 5 | 0x8000;
        }
        if (!sendDataAvail(&queueItem->pendingdata)) {
            wsErr("SDA failed: could not hand image to C6 radio (check S3 UART pins 17/18)");
        }
    }
}
...
static String kioskImagePath(const uint8_t mac[8]) {
    char hexmac[17];
    mac2hex(mac, hexmac);
    return "/kiosk/" + String(hexmac) + ".raw";
}

bool stageKioskFromJpeg(const uint8_t* mac, const String& jpgPath, uint8_t dither) {
    ...
    imageParams.zlib = (hwdata.zlib != 0 && taginfo->tagSoftwareVersion >= hwdata.zlib) ? 1 : 0;
    imageParams.g5 = 0;
    imageParams.lut = EPD_LUT_NO_REPEATS;

    char hexmac[17];
    mac2hex(mac, hexmac);
    String inPath = jpgPath;
    if (!inPath.startsWith("/")) {
        inPath = "/" + inPath;
    }
    if (!contentFS->exists(inPath)) {
        return false;
    }
    String rawTemp = "/temp/" + String(hexmac) + ".raw";
    contentFS->remove(rawTemp);
    jpg2buffer(inPath, rawTemp, imageParams);
    if (!contentFS->exists(rawTemp)) {
        return false;
    }
    String dst = "/kiosk/" + String(hexmac) + ".raw";
    contentFS->remove(dst);
    if (!contentFS->rename(rawTemp, dst)) {
        return false;
    }
    return true;
}

bool kioskImageIsStaged(const uint8_t* mac) {
    return contentFS->exists(kioskImagePath(mac));
}

bool pushKioskStagedImage(const uint8_t* mac) {
    String path = kioskImagePath(mac);
    if (!contentFS->exists(path)) {
        return false;
    }
    const uint8_t typeArg = 0;  /* CUSTOM_IMAGE_NOCUSTOM — standard OEPL content */
    return prepareDataAvail(path, DATATYPE_IMG_ZLIB, typeArg, mac, 0);
}

bool pushKioskImageBuffer(const uint8_t* mac, uint8_t* data, size_t len, const char* pin) {
    ...
    if (!tagContentPushAllowed(taginfo) &&
        (pin == nullptr || strlen(pin) != 6)) {
        free(data);
        return false;
    }
    ...
    taginfo->data = data;
    taginfo->len = len;
    taginfo->dataType = DATATYPE_IMG_ZLIB;
    taginfo->filename = String();
    taginfo->pendingCount++;
    taginfo->pendingIdle = 5;

    struct pendingData pending = {0};
    memcpy(pending.targetMac, mac, 8);
    pending.availdatainfo.dataType = DATATYPE_IMG_ZLIB;
    pending.availdatainfo.dataTypeArgument = 0;  /* NOCUSTOM */
    pending.availdatainfo.dataVer = *((uint64_t*)md5bytes);
    pending.availdatainfo.dataSize = len;
    pending.availdatainfo.nextCheckIn = 5 | 0x8000;
    if (pin != nullptr && strlen(pin) == 6) {
        for (int i = 0; i < 6; i++) {
            pending.availdatainfo.provisionPin[i] = (uint8_t)pin[i];
        }
    }
    pending.attemptsLeft = 8;
