...
bool tagUsesKioskWorkflow(const tagRecord* taginfo) {
    if (taginfo == nullptr) {
        return false;
    }
    if (taginfo->lastCustomMode == TAG_CUSTOM_MODE_PROVISIONING) {
        return true;
    }
    if (taginfo->lastCustomMode == TAG_CUSTOM_MODE_PROVISION_AUTHORIZED) {
        return true;
    }
    if (taginfo->lastCustomMode == TAG_CUSTOM_MODE_KIOSK_UNPROVISIONED) {
        return true;
    }
    ...
    /* pinprov tag FW (v70+) stays PIN-gated until explicitly unlocked */
    if (taginfo->tagSoftwareVersion >= 70 && taginfo->updatesUnlocked == 0) {
        return true;
    }
    return false;
}

bool tagContentPushAllowed(const tagRecord* taginfo) {
    if (taginfo == nullptr) {
        return false;
    }
    ...
    if (taginfo->provisioned == 0) {
        return false;
    }
    return taginfo->updatesUnlocked != 0;
}

bool tagNeedsPinForImagePush(const tagRecord* taginfo) {
    if (taginfo == nullptr) {
        return false;
    }
    return tagUsesKioskWorkflow(taginfo) && !tagContentPushAllowed(taginfo);
}

bool tagAwaitingProvision(const tagRecord* taginfo) {
    return tagNeedsPinForImagePush(taginfo);
}

#define PROVISION_SESSION_TIMEOUT_MS (3UL * 60UL * 1000UL)

void expireStaleProvisionSessions() {
    const uint32_t now = millis();
    for (tagRecord* taginfo : tagDB) {
        if (taginfo == nullptr || taginfo->version != 0 || taginfo->pinSessionOpen == 0) {
            continue;
        }
        if (taginfo->pinSessionStarted == 0) {
            taginfo->pinSessionStarted = now;
            continue;
        }
        if ((now - taginfo->pinSessionStarted) < PROVISION_SESSION_TIMEOUT_MS) {
            continue;
        }

        char hexmac[17];
        mac2hex(taginfo->mac, hexmac);
        wsErr(String("Provision session timed out for ") + hexmac);
        prepareCancelPending(taginfo->mac);
        while (dequeueItem(taginfo->mac)) {
        }
...
void updateTagProvisionState(tagRecord* taginfo, uint8_t customMode, const uint8_t* reserved) {
    (void)reserved;
    if (taginfo == nullptr) {
        return;
    }
    taginfo->lastCustomMode = customMode;
    if (customMode == TAG_CUSTOM_MODE_PROVISIONING) {
        return;
    }
    if (customMode == TAG_CUSTOM_MODE_KIOSK_UNPROVISIONED) {
        taginfo->provisioned = 0;
        taginfo->updatesUnlocked = 0;
        taginfo->pinSessionOpen = 0;
        return;
    }
    if (customMode == TAG_CUSTOM_MODE_PROVISION_AUTHORIZED) {
...
void fillNode(JsonObject& tag, const tagRecord* taginfo) {
    ...
    tag["provisioned"] = taginfo->provisioned;
    tag["updatesUnlocked"] = taginfo->updatesUnlocked;
    if (taginfo->lastCustomMode == TAG_CUSTOM_MODE_PROVISIONING && taginfo->pinSessionOpen == 0) {
        tag["awaitingPin"] = true;
    }
    if (tagNeedsPinForImagePush(taginfo)) {
        tag["needsUploadPin"] = true;
    }
    if (taginfo->provisioned != 0 && taginfo->updatesUnlocked == 0) {
        tag["updatesLocked"] = true;
    }
    if (taginfo->pinSessionOpen != 0) {
        tag["programming"] = true;
        tag["provisionState"] = "programming";
    } else if (taginfo->lastCustomMode == TAG_CUSTOM_MODE_PROVISIONING) {
        tag["provisionState"] = "pin_visible";
    } else if (taginfo->lastCustomMode == TAG_CUSTOM_MODE_KIOSK_UNPROVISIONED) {
        tag["provisionState"] = "kiosk_idle";
    } else if (taginfo->provisioned != 0 && taginfo->updatesUnlocked == 0) {
        tag["provisionState"] = "locked";
    } else if (taginfo->provisioned == 0) {
        tag["provisionState"] = "kiosk_idle";
    } else {
        tag["provisionState"] = "idle";
    }
    tag["kioskStaged"] = false;
}
        ...
        while (parsing) {
            ...
            if (!err) {
                ...
                if (hex2mac(dst, mac)) {
                    ...
                    if (tag["provisioned"].isNull()) {
                        taginfo->provisioned = 1;
                    } else {
                        taginfo->provisioned = tag["provisioned"] | 0;
                    }
                    if (tag["updatesUnlocked"].isNull()) {
                        taginfo->updatesUnlocked = taginfo->provisioned ? 1 : 0;
                    } else {
                        taginfo->updatesUnlocked = tag["updatesUnlocked"] | 0;
                    }
                }
            } else {
